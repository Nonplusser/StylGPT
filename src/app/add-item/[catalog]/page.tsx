...
export default function CatalogPage({ params }: { params: { catalog: string } }) {
  ...
  const catalogName = useMemo(() => decodeURIComponent(params.catalog), [params.catalog]);

  useEffect(() => {
    const fetchImages = async () => {
      setLoading(true);
      // Construct the folder path based on the dynamic catalog name
      const folderPath = `Public-Catalog/${catalogName}/`;
      const urls = await getPublicCatalogImages(folderPath);
      setImageUrls(urls);
      setLoading(false);
    };
  ...
  const handleAddSelectedToWardrobe = async () => {
    ...
    try {
      const itemsToAdd = selectedUrls.map(url => ({
        photoUrl: url,
        storagePath: `public-catalog-imports/${user.uid}/${uuidv4()}`,
      }));
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-3xl font-bold font-headline">{catalogName.replace(/[-_]/g, ' ')}</h1>
        </div>
